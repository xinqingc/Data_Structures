...
void printSmallerValues(BTNode *node, int m){
	...
	if (node!=NULL)
    {
        //below pre-order, in-order is left-centre-right,
        if (node->item<m)
            printf("%d", node->item);
...
int hasGreatGrandchild(BTNode *node){
    ...
    {
        l = 1+ hasGreatGrandchild(node->left);//height
        r = 1+ hasGreatGrandchild(node->right);
    }
    if (l>=3 || r>=3)
        printf("%d ",node->item);
    if (l>=r)
