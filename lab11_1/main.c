...
int main()
{
	...
	//for question 4
	if (balanced("()")) printf("not balanced!\n"); //return 0 (balanced) is false, return -1 (not balanced) is true,
	else
		printf("balanced!\n");
...
void recursiveReverse(Queue *q){
	...
	int item;
	//no need put if else when if does nothing
	if (!isEmptyQueue(q))
    {
        item = dequeue(q);
        recursiveReverse(q); // excludes the removed element
        enqueue(q, item);
    }
}
...
int palindrome(char *word){
	...
	while (*word !='\0')
    {
        ...
        {
            push(&s, toupper(*word)); //convert to uppercase
            enqueue(&q, toupper(*word)); //function need to include <ctype.h>
        }
        word++;
    }
    while (!isEmptyStack(&s))
...
int balanced(char *expression){
	...
	//see open bracket, push, close bracket, pop and compare to popped bracket
	Stack s;
